<header class="w-full">

    <nav class="bg-white border-gray-200 py-2.5">
        <div class="flex flex-wrap items-center justify-between max-w-screen-xl px-4 mx-auto">
            <a href="#" class="flex items-center">
                <img src="{{ asset('backend/assets/media/logos/Nissan_2020_logo.svg') }}" class="h-10 mr-3 sm:h-16"
                     alt="Nissan"/>
            </a>

            <div class="items-center justify-between hidden w-full lg:flex lg:w-auto lg:order-1 lg:ml-auto"
                 id="mobile-menu-2">
                <ul class="flex flex-col mt-4 font-small lg:flex-row lg:space-x-8 lg:mt-0">
                    <li>
                        <a href="#"
                           class="block py-2 pl-3 pr-4 text-black lg:bg-transparent lg:p-0 text-transform: uppercase text-sm duration-200 ease-in-out transition-transform  hover:scale-102 hover:opacity-75 custom"
                           aria-current="page">Vehículos</a>
                    </li>
                    <li>
                        <a href="#"
                           class="block py-2 pl-3 pr-4 text-black hover:bg-gray-50 lg:hover:bg-transparent lg:border-0 lg:p-0 text-transform: uppercase text-sm transition-colors duration-200 ease-in-out transition-transform transition-opacity hover:scale-102 hover:opacity-75 custom">Showroom
                            Virtual</a>
                    </li>
                    <li class="relative group">
                        <button
                            class="inline-flex items-center py-2 pl-3 pr-4 text-black lg:bg-transparent lg:p-0 text-transform: uppercase text-sm transition-colors duration-200 ease-in-out custom cursor-pointer">
                            CITAS DE SERVICIO
                            <!-- Dropdown arrow icon -->
                            <svg xmlns="http://www.w3.org/2000/svg" class="ml-2 h-5 w-5" viewBox="0 0 20 20"
                                 fill="currentColor" aria-hidden="true">
                                <path
                                    d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z"/>
                            </svg>
                        </button>
                        <!-- Dropdown menu -->
                        <div
                            class="absolute hidden group-hover:block bg-white text-gray-900 rounded shadow-lg p-5  w-72 z-50 text-xss"
                            style="margin-left: -80px">
                            <a href="#" class="block px-4 py-2 text-sm hover:bg-gray-100">Solicitar cita por
                                WhatsApp</a>
                            <a href="#" class="block px-4 py-2 text-sm hover:bg-gray-100">Agendar cita en el sistema
                                automático</a>
                        </div>
                    </li>
                    <li class="relative group">
                        <button
                            class="inline-flex items-center py-2 pl-3 pr-4 text-black lg:bg-transparent lg:p-0 text-transform: uppercase text-sm transition-colors duration-200 ease-in-out custom cursor-pointer">
                            posventa
                            <!-- Dropdown arrow icon -->
                            <svg xmlns="http://www.w3.org/2000/svg" class="ml-2 h-5 w-5" viewBox="0 0 20 20"
                                 fill="currentColor" aria-hidden="true">
                                <path
                                    d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z"/>
                            </svg>
                        </button>
                        <!-- Dropdown menu -->
                        <div
                            class="absolute hidden group-hover:block bg-white text-gray-900 rounded shadow-lg p-5  w-72 z-50 text-xss"
                            style="margin-left: -80px">
                            <a href="#" class="block px-4 py-2 text-sm hover:bg-gray-100">Descubre accesorios
                                ogiginales</a>
                            <a href="#" class="block px-4 py-2 text-sm hover:bg-gray-100">Cotiza repuestos
                                originales</a>
                            <a href="#" class="block px-4 py-2 text-sm hover:bg-gray-100">Ofertas en Shopping
                                Posventa</a>
                            <a href="#" class="block px-4 py-2 text-sm hover:bg-gray-100">Direcciones de centros de
                                servicio y repuestos</a>
                            <a href="#" class="block px-4 py-2 text-sm hover:bg-gray-100">Nissan renta</a>
                            <a href="#" class="block px-4 py-2 text-sm hover:bg-gray-100">Campañas Recall</a>
                        </div>
                    </li>
                    <li>
                        <a href="#"
                           class="block py-2 pl-3 pr-4 text-black hover:bg-gray-50 lg:hover:bg-transparent lg:border-0 lg:p-0 text-transform: uppercase text-sm transition-colors duration-200 ease-in-out transition-transform transition-opacity hover:scale-102 hover:opacity-75 custom">seminuevos</a>
                    </li>
                    <li class="relative group">
                        <button
                            class="inline-flex items-center py-2 pl-3 pr-4 text-black lg:bg-transparent lg:p-0 text-transform: uppercase text-sm transition-colors duration-200 ease-in-out custom cursor-pointer">
                            CITAS DE SERVICIO
                            <!-- Dropdown arrow icon -->
                            <svg xmlns="http://www.w3.org/2000/svg" class="ml-2 h-5 w-5" viewBox="0 0 20 20"
                                 fill="currentColor" aria-hidden="true">
                                <path
                                    d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z"/>
                            </svg>
                        </button>
                        <!-- Dropdown menu -->
                        <div
                            class="absolute hidden group-hover:block bg-white text-gray-900 rounded shadow-lg p-5  w-72 z-50 text-xss"
                            style="margin-left: -140px">
                            <a href="#" class="block px-4 py-2 text-sm hover:bg-gray-100">Esto es Nissan</a>
                            <a href="#" class="block px-4 py-2 text-sm hover:bg-gray-100">Nissan Intelligent
                                Mobility</a>
                            <a href="#" class="block px-4 py-2 text-sm hover:bg-gray-100">Desempeño Nismo</a>
                            <a href="#" class="block px-4 py-2 text-sm hover:bg-gray-100">Novedades</a>
                        </div>
                    </li>
                </ul>

            </div>
            <div class="flex items-center lg:order-2">
                <button id="hamburger-button" class="p-4 focus:outline-none focus:bg-gray-100" onclick="toggleMenu()">
                    <!-- Hamburger Icon -->
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24"
                         stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                              d="M4 6h16M4 12h16m-7 6h7"/>
                    </svg>
                </button>
            </div>
            <div id="menu-modal"
                 class="fixed inset-y-0 right-0 transform translate-x-full transition-transform duration-300 ease-in-out bg-white shadow-md w-80 space-y-6 py-7 px-5 z-50">
                <div class="flex justify-end mb-20">
                    <button class="p-2 focus:outline-none focus:bg-gray-100" onclick="toggleMenu()">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24"
                             stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                  d="M6 18L18 6M6 6l12 12"/>
                        </svg>
                    </button>
                </div>
                <!-- Menu Content -->
                <a href="https://www.nissan.com.bo/calculadora.html"
                   class="block font-medium text-sm text-gray-700 hover:bg-gray-50 rounded-md px-2 mt-20">Calculadora de
                    créditos</a>
                <a href="https://www.nissan.com.bo/buscar-concesionario.html"
                   class="block font-medium text-sm text-gray-700 hover:bg-gray-50 rounded-md px-2">Direcciones y
                    teléfonos</a>
                <a href="https://www.nissan.com.bo/vehiculos/catalogos.html"
                   class="block font-medium text-sm text-gray-700 hover:bg-gray-50 rounded-md px-2">Catálogos</a>
                <a href="https://technoaid.startlogic.com/paginasweb/formulariosnissan/index.php?r=test/create"
                   class="block font-medium text-sm text-gray-700 hover:bg-gray-50 rounded-md px-2">Solicita una Prueba
                    de Manejo</a>
                <a href="https://cotizaciones.contacto-nissan.com.bo/"
                   class="block font-medium text-sm text-gray-700 hover:bg-gray-50 rounded-md px-2">Solicita una
                    cotización de vehículos</a>
                <a href="https://wa.me/59161011116?text=Iniciar%20web%20cotizacion%20de%20repuestos"
                   class="block font-medium text-sm text-gray-700 hover:bg-gray-50 rounded-md px-2">Cotiza repuestos
                    originales</a>
                <a href="https://www.nissan.com.bo/clientes/manuales.html"
                   class="block font-medium text-sm text-gray-700 hover:bg-gray-50 rounded-md px-2">Manuales y Guías</a>

            </div>
        </div>
    </nav>

</header>
